    ...
    def decorator(cls: _DataObjectBaseT) -> _DataObjectBaseT:
        ...
        if not issubclass(cls, Enum):
            ...
            for annotation in getattr(cls, "__annotations__", {}):
                # If this class is a dataclass and this field has dataclass specific field defaults then
                # use those
                dataclass_defined_default = data_class_fields.get(annotation, dataclasses.MISSING)
                if dataclass_defined_default != dataclasses.MISSING:
                    if dataclass_defined_default.default != dataclasses.MISSING:
                        user_defined_defaults[annotation] = dataclass_defined_default.default
                        continue
                    if dataclass_defined_default.default_factory != dataclasses.MISSING:
                        user_defined_defaults[annotation] = dataclass_defined_default.default_factory
                        continue
                
                # Fallback to generic user defined defaults
                user_defined_default = getattr(cls, annotation, dataclasses.MISSING)
                if user_defined_default == dataclasses.MISSING:
                    log.debug3("Filling in None default for %s.%s", cls, annotation)  # type: ignore
            ...
            setattr(cls, _DataBaseMetaClass._USER_DEFINED_DEFAULTS, user_defined_defaults)
...
class _DataobjectConverter(DataclassConverter):
    ...
    def get_optional_field_names(self, entry: Any) -> List[str]:
        ...
        """
        optional_fields = list(getattr(entry, _DataBaseMetaClass._USER_DEFINED_DEFAULTS, {}))
